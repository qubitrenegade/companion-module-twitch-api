...
export const waitUntil = async (timestampMs: number, signal: AbortSignal): Promise<void> => {
  const abortReason = (): Error => (signal.reason instanceof Error ? signal.reason : new DOMException(signal.reason == null ? 'Aborted' : String(signal.reason), 'AbortError'))
...
export class RaidBrowser {
  ...
  public async refreshAndSelectDefault(suggestionText = ''): Promise<void> {
    ...
    const identityGeneration = this.instance.auth.identityGeneration
    const selectedChannel = this.instance.channels.find((channel) => channel.username === this.instance.selectedChannel)
    const suppliedText = suggestionText.trim()
    const currentBroadcasterTitle = suppliedText ? '' : await this.loadAuthenticatedBroadcasterTitle()
    if (refreshGeneration !== this.refreshGeneration || identityGeneration !== this.instance.auth.identityGeneration || !this.instance.auth.valid) return

    const sourceText = suppliedText || currentBroadcasterTitle || selectedChannel?.title || ''
    ...
    if (suggestedIndex >= 0) {
      ...
      this.instance.log('debug', 'No explicit up next raid candidate was found; selected candidate 1')
    }
  }

  private async loadAuthenticatedBroadcasterTitle(): Promise<string> {
    const controller = new AbortController()

    try {
      const response = await this.loadSourceWithTimeout(
        async (signal) => this.getHelix<TwitchChannel>(`channels?broadcaster_id=${encodeURIComponent(this.instance.auth.userID)}`, signal),
        controller.signal,
      )
      ...
      const message = error instanceof Error ? error.message : String(error)
      this.instance.log('warn', `Unable to load the authenticated broadcaster title for raid selection: ${message}`)
      return ''
    }
  }
  ...
  private async getHelix<T>(path: string, signal: AbortSignal, mayRetry = true): Promise<HelixResponse<T>> {
    ...
    return body as unknown as HelixResponse<T>
  }
}
