...
describe('follower-mode chat settings', () => {
  ...
  test.each([
    ['slow_mode_wait_time', 'slow mode'],
    ['follower_mode_duration', 'follower mode'],
  ])('rejects a non-numeric %s value before calling Twitch', async (mode, description) => {
    const instance = makeInstance()

    await updateChatSettings(instance, 'channel', mode, 'abc')

    expect(fetchMock).not.toHaveBeenCalled()
    expect(instance.log).toHaveBeenCalledWith('warn', expect.stringContaining(`${description} requires a finite numeric duration`))
  })
})
