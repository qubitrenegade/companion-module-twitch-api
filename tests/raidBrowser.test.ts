...
describe('raid candidate transforms', () => {
  test('preserves the reason that interrupts a rate-limit wait', async () => {
    const controller = new AbortController()
    const reason = new Error('Authentication changed')
    const rejection = expect(waitUntil(Date.now() + 60_000, controller.signal)).rejects.toBe(reason)
  ...
  test('extracts only an explicit up-next Twitch login', () => {
    ...
    expect(parseSuggestedRaidLogin('The 🥜 And Bass Locomotive | Happy Birthday @dj5qrl 🥜| Up Next @wildlifecam')).toBe('wildlifecam')
...
describe('RaidBrowser Twitch loading', () => {
  ...
  test('clears candidates and prevents an in-flight refresh from restoring state after reauthentication begins', async () => {
    const { instance, browser } = makeInstance()
    instance.raidCandidates = [candidate('old-account')]
    const request = deferred<Response>()
    fetchMock.mockReturnValueOnce(request.promise)

    const staleRefresh = browser.refresh()
    instance.auth.valid = false
    browser.authenticationInvalidated()
  ...
  test('refreshes and selects an up-next candidate from the authenticated broadcaster title', async () => {
    const { instance, browser } = makeInstance()
    fetchMock
      .mockResolvedValueOnce(response({ data: [stream('2', 20), stream('1', 10)], pagination: {} }))
      .mockResolvedValueOnce(response({ data: [{ title: 'Up next: @user1' }] }))

    await browser.refreshAndSelectDefault()

    expect(instance.raidCandidates[instance.raidCandidateIndex]?.login).toBe('user1')
    expect(fetchMock.mock.calls[1][0]).toBe('https://api.twitch.tv/helix/channels?broadcaster_id=self')
  ...
  test('refreshes and selects candidate 1 when no up-next target is live', async () => {
    ...
    fetchMock
      .mockResolvedValueOnce(response({ data: [stream('2', 20), stream('1', 10)], pagination: {} }))
      .mockResolvedValueOnce(response({ data: [{ title: 'Up next: @offline_user' }] }))
...
describe('raid browser integration boundaries', () => {
  ...
  test('Raid Browser: Acknowledge Error clears the retained operator error', async () => {
    ...
    const instance = {
      ...
      API: {},
      raidBrowser: {},
      raidState: { clearError },
      chat: {},
      log: jest.fn(),
    } as unknown as TwitchInstance
    const actions = getActions(instance)

    await actions.raidClearError.callback({ options: {} } as never, {} as never)
